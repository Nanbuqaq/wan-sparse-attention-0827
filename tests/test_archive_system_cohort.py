...
def test_archive_factors_have_matched_source_controls_and_reversed_order(tmp_path):
    ...
    lanes=case_lane_indices(rows,2);assert list(map(len,lanes))==[10,10]
    for lane,ids in enumerate(lanes):
        items=[rows[i] for i in ids]
        assert [r['method'] for r in items]==['native_start','sync_r0','serial_r0','async_device_r0','async_stream_r0','async_stream_r1','async_device_r1','serial_r1','sync_r1','native_end']
        for row in items:
