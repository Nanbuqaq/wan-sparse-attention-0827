...
if [[ -n ${NATIVE_GEOMETRY_INPUTS:-} ]]; then
  export PYTHONPATH="$INFER_WEIGHTS_DIR/$NATIVE_GEOMETRY_INPUTS/sam2-runtime:$INFER_WEIGHTS_DIR/$NATIVE_GEOMETRY_INPUTS/sam2-python:$PYTHONPATH"
fi
...
if [[ -n ${NATIVE_GEOMETRY_INPUTS:-} ]]; then
  python scripts/qualify_replayed_source_geometry.py --replay "$INFER_OUTPUT_DIR/replay" \
    --checkpoint "$INFER_WEIGHTS_DIR/$NATIVE_GEOMETRY_INPUTS/sam2_hiera_large.pt" --output "$INFER_OUTPUT_DIR/geometry"
fi
