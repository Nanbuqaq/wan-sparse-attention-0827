...
def main():
    ...
    report=json.loads((args.case/'summary.json').read_text());d=torch.load(args.case/'wave2_diagnostics.pt',weights_only=True,map_location='cpu')['call']
    ...
    result=dict(status='pass',rows=rows,heads=q.shape[1],queries_per_head=len(sites),groups=len(costs),budget_per_head=budget,
