...
if [[ ${#assigned_gpus[@]} -lt 2 ]]; then
  echo "Dense prompt screen requires two assigned GPUs" >&2
  exit 2
fi
...
native_output=${batch_root}/native_dense
rag_output=${batch_root}/rag_dense
mkdir -p "${native_output}" "${rag_output}"

CUDA_VISIBLE_DEVICES=${assigned_gpus[0]} INFER_OUTPUT_DIR=${native_output} \
  python scripts/run_loaded_dense_screen.py \
    --runtime native_dense --base-config configs/inferhub/native_dense_21.yaml \
    >"${batch_root}/native_dense.log" 2>&1 & pid0=$!
CUDA_VISIBLE_DEVICES=${assigned_gpus[1]} INFER_OUTPUT_DIR=${rag_output} \
  python scripts/run_loaded_dense_screen.py \
    --runtime rag_dense --base-config configs/inferhub/rag_dense_21.yaml \
    >"${batch_root}/rag_dense.log" 2>&1 & pid1=$!
...
payload = {"native_dense_status": ${status0}, "rag_dense_status": ${status1}}
