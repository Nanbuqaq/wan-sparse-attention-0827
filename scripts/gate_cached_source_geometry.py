...
def main():
    ...
    for version in (2,1,3,2):
        try:
            ...
            if version==2:
                ...
                if not row['exact_pixel_masks_to_existing_reference']:raise RuntimeError('shared model changed source masks')
    ...
    report=dict(status='pass',rows=rows,model_load_s=model.load_s,model_tensor_bytes=model.model_tensor_bytes,
