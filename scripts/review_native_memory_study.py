...
def main():
    ...
    terminal=collect(args.root);groups=[];sections=[]
    for lane in range(4):
        ...
        for arm in ARMS:
            ...
            indices=[173,189,313,377,385,405,445,505]
        ...
        parts=[f'<h2>组 {lane}</h2><img src="lane{lane}__comparison.jpg" width="100%">']
        for row in rows:
            links=' · '.join(f'<a href="{html.escape(path)}">{html.escape(name)}</a>' for name,path in row.get('boards',{}).items())
            parts.append(f'<p>{html.escape(row["arm"])}：{links}</p>')
        sections.append(''.join(parts))
    report=dict(terminal=terminal,groups=groups,blind_human_review=False,semantic_review_complete=False)
