...
def main():
    ...
    report = dict(status='running', prompt=prompt, seed=20260904, latent_frames=args.latent_frames,
        ...
        interpretation='causal effect reference with full-candidate moment construction; all overhead charged')
    ...
    for name, mode, block in variants:
        ...
        try:
            ...
            record = dict(variant=name,status='pass',effective_method=name,base_raw_selector='legacy_Final_70_15_15',
                prototype_block_tokens=block if mode == 'prototype_tail' else None,
