...
def build_cohort(spec,stage,assets,source,output,seed,base_builder):
    ...
    if stage=='shared_route125_reuse_pipeline4_compile_long728':
        ...
        return rows
    if stage in ('shared_route_repeats','shared_route125_repeats','shared_route125_reuse_repeats','shared_route0625_reuse_repeats','shared_route125_reuse_pipeline4_repeats','shared_route125_reuse_pipeline4_compile_repeats','shared_route125_reuse_pipeline4_compilemax_repeats'):
        ...
        candidate_label=('shared25' if stage=='shared_route_repeats' else
            'shared125_reuse' if stage in ('shared_route125_reuse_repeats','shared_route125_reuse_pipeline4_repeats','shared_route125_reuse_pipeline4_compile_repeats','shared_route125_reuse_pipeline4_compilemax_repeats') else
        ...
        pipeline4=stage in ('shared_route125_reuse_pipeline4_repeats','shared_route125_reuse_pipeline4_compile_repeats','shared_route125_reuse_pipeline4_compilemax_repeats')
        compile_decoder=stage in ('shared_route125_reuse_pipeline4_compile_repeats','shared_route125_reuse_pipeline4_compilemax_repeats')
        cases=[]
        for task_index,task in enumerate(tasks):
            template=next(c for c in base if c['scenario']==task)
            for repeat in range(3):
                order=(candidate_label,'native') if (repeat+task_index)%2==0 else ('native',candidate_label)
                for label in order:
                    ...
                    if compile_decoder:
                        ...
                        if stage=='shared_route125_reuse_pipeline4_compilemax_repeats':
                            cmd+=['--pipeline-compile-decoder-mode','max-autotune-no-cudagraphs']
