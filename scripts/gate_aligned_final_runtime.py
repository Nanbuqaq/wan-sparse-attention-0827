...
def run(large, bootstrap_layer=None, layer_id=0, single_candidate=False):
    ...
    candidate_count=1 if single_candidate else history
    cfg=SparseHistoryConfig(method='rope_aligned_final_history' if bootstrap_layer is None else 'rope_bootstrap_ablation_history',
        history_density=.25,refresh_policy='per_chunk',method_params={} if bootstrap_layer is None else {'bootstrap_layer':bootstrap_layer})
    aligned_space=cfg.uses_aligned_prototypes(layer_id,candidate_count)
    ...
    for frame in range(1,history+1):
        ...
        archive.index_frame(layer_id,frame,k,v,storage_k=raw_k,storage_v=raw_v)
        rotated=archive_rope0_key(k,spatial_height=height,spatial_width=width,freqs=freqs)
        control.index_frame(layer_id,frame,rotated if aligned_space else k,raw_v,storage_k=raw_k,storage_v=raw_v)
    ...
    module=runtime.SparseHistorySelfAttention(dim=heads*dim,num_heads=heads,local_attn_size=local,
        sink_size=1,memory_size=history,layer_id=layer_id,history_archive=archive,sparse_config=cfg,
    ...
    try:
        ...
        raw_first_query=module.norm_q(module.q(x)).view(1,new*tokens,heads,dim)
        for call in range(5):
            output,update=module(x+call*.1,torch.tensor([new*tokens],device='cuda'),
                ...
                current_start=current,memory_indices=torch.arange(candidate_count,device='cuda').view(1,-1))
    ...
    first_query=captures[0][0] if aligned_space else raw_first_query
    reference_plan=control.route_indexed(layer_id,summarize_query_for_pretransfer(first_query,64),
                                        list(range(1,candidate_count+1)),exact_k_tokens=captures[0][1].shape[1])
    dense_k,dense_v,frame_ids,token_ids=control.dense_history_tensors(layer_id,list(range(1,history+1)))
    ...
    for call,(q,ek,ev,actual,plan) in enumerate(captures):
        ...
        if match['jaccard']!=1. or plan.metadata['query_summary_space']!=('post_rope' if aligned_space else 'unrotated'):
    ...
    for frame in archive.frame_ids(layer_id):
        actual=archive._layers[layer_id][frame]
        ...
        if cfg.needs_bootstrap_raw(layer_id):
            raw_view=actual.key.cuda().permute(0,2,1,3)
            raw_prototypes=torch.stack([raw_view[:,:,start:start+64].float().mean(2)
                                        for start in range(0,tokens,64)],dim=2).cpu()
            if not torch.equal(actual.block_unrotated_centroids,raw_prototypes):
                raise RuntimeError('bootstrap raw prototype mismatch')
    result={'status':'pass' if all(r['pass'] for r in records) else 'fail',
        ...
        'bootstrap_layer':bootstrap_layer,'layer_id':layer_id,'candidate_frames':candidate_count,
...
def main():
    ...
    parser.add_argument('--bootstrap-layer',type=int)
    ...
    result=run(args.large,args.bootstrap_layer,args.layer,args.single_candidate)
