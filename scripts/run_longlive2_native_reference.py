...
def main():
    ...
    p.add_argument('--wave2-route-refresh',choices=('every_step','first_only','dual_02','causal_age'),default='every_step')
    ...
    p.add_argument('--pipeline-compile-decoder-mode',choices=('default','max-autotune-no-cudagraphs'),default='default')
    ...
    if args.wave2_query_pack_backend!='torch' and not args.wave2_query_groups:
        raise ValueError('fused query packing requires an explicit frame-query routing policy')
    ...
    if args.source_lifetime_study:
        if (args.wave2_method not in ('w2_native','w2_full_recall')
            or args.cut_scenario not in ('w2_ceramic_jug_revisit','generated_patchwork_toy_cut_revisit',*STATE_SCENARIOS)
    ...
    if args.state_past_request_text and (args.cut_scenario!='w2_state_red_toolbox_last_open'
        ...
        or (not args.state_raw_text_combination and (not args.source_no_archive or args.source_lifetime_policy!='off'))):
        raise ValueError('past-state text control requires its qualified archive-free opening history')
    if args.state_raw_text_combination and (not args.state_past_request_text
    ...
    continuous_duration=(args.duration_probe_latents in ((96,) if args.gate else (728,3608,14429))
    ...
    if args.gate and episode_layout:
        length=128 if args.cut_scenario in WRITE_SCENARIOS or args.source_snapshot_preserve_gate_timeline else 80 if args.source_lifetime_study else 64
    ...
    try:
        ...
        if args.pipeline_mode!='none':
            ...
            if args.pipeline_compile_decoder:
                ...
                warmup_compiled_decoder(pipe.vae.model,(latent_height,latent_width),torch.device('cuda:1'),mode=args.pipeline_compile_decoder_mode)
        ...
        if args.wave2_method:
            ...
            if args.wave2_method=='w2_native':
                ...
                if args.version_read!='all' or args.version_kv_role is not None or args.version_kv_role_staged is not None:
                    from adapters.longlive_sparse.version_read_control import VersionReadControl
                    controller_type=VersionReadControl;controller_kwargs=dict(version_read=args.version_read,version_kv_role=args.version_kv_role,version_kv_role_staged=args.version_kv_role_staged)
                ...
                resident_history=controller_type(pipe,args.wave2_method,fraction=args.wave2_steady_fraction,
                    ...
                    query_pack_backend=args.wave2_query_pack_backend,route_age_limit=args.wave2_route_age_limit,
                    transition_anchor=args.wave2_transition_anchor,
        ...
        if args.pipeline_mode!='none':
            ...
            video_pipeline=NativeVideoPipeline(pipe.vae,unpatchify,pipeline_sink,source_device='cuda:0',target_device='cuda:1',
                ...
                compile_decoder=args.pipeline_compile_decoder,compile_mode=args.pipeline_compile_decoder_mode,
