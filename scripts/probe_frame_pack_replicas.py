...
def main():
    ...
    for case in a.cases:
        ...
        for c in data['calls']:
            ...
            for policy in ['split_shared','split_specific']:
                ...
                rk_input=k.repeat(1,g,1,1);rv_input=v.repeat(1,g,1,1)
                ids=plan['frame_ids']+torch.arange(g,device=k.device)[:,None,None]*(k.shape[1]//880)
                for rep in range(9):
                    for adjacent in ([True,False] if rep%2==0 else [False,True]):
                        ...
                        pack[grid](rk_input,rv_input,ids,ok,ov,880,h,128,nf,adjacent,16,num_warps=4)
                ...
                row=dict(case=case.name,frame=c['frame'],policy=policy,packed_KV_bytes=2*rk.numel()*rk.element_size(),
                    same_output_bitwise=True,deliberate_input_replication_not_a_real_long_history=True,replica_setup_excluded_from_timing=True,extra_GPU_replica_bytes=(g-1)*2*k.numel()*k.element_size(),heads_adjacent_ms=statistics.median(times[True]),tokens_adjacent_ms=statistics.median(times[False]),samples={str(k):v for k,v in times.items()})
                rows.append(row);print(json.dumps(row),flush=True);del rk,rv,ok,ov,rk_input,rv_input
    with (a.output/'result.json').open('x') as f:json.dump(dict(status='pass',GPU=torch.cuda.get_device_name(),rows=rows,
        scope='same values and outputs with deliberately disjoint per-group physical input replicas; mechanistic counterfactual, not a deployable baseline, storage saving or long-video evidence'),f,indent=2)
