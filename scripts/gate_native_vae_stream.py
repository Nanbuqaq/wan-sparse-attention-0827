...
def main():
    ...
    p.add_argument('--original-resolution',action='store_true')
    ...
    expected_latents=[1,128,48,44,80] if args.original_resolution else [1,64,48,16,32]
    assert list(latent.shape)==expected_latents
    ...
    batch_peak_allocated=torch.cuda.max_memory_allocated();batch_peak_reserved=torch.cuda.max_memory_reserved()
    ...
    for chunk_size in (8,7,13):
        ...
        assert offset==expected_shape[1]
        ...
        rows.append(dict(chunk_size=chunk_size,latent_frames=latent.shape[1],pixel_frames=offset,numeric_gate=passed,
    ...
    result=dict(status='pass' if original_rgb_exact and all(r['numeric_gate'] for r in rows) else 'fail',
        ...
        latent_shape=expected_latents,original_resolution=args.original_resolution,
        batch_GPU_peak_allocated_bytes=batch_peak_allocated,batch_GPU_peak_reserved_bytes=batch_peak_reserved,
