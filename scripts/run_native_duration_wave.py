...
def build_wave2_cases(spec,stage,assets,source,output,seed,valid_scenarios=None,expected_noise=None):
    if stage=='global_payload_replication':
        from scripts.global_payload_replication_cohort import build_global_payload_replication
        if expected_noise:raise ValueError('global role replication requires current hardware input checks')
        return build_global_payload_replication(spec,assets,source,output,seed,build_wave2_cases)
    if stage=='archive_combined':
        from scripts.combined_archive_cohort import build_combined_archive
        if expected_noise:raise ValueError('combined archive wave requires its own hardware input gate')
        return build_combined_archive(spec,assets,source,output,seed,build_wave2_cases)
    ...
    if stage in ('matched_controls','timing_repeats','shared_route_repeats','shared_route125_repeats','shared_route125_reuse_repeats','shared_route0625_reuse_repeats','shared_route125_reuse_pipeline4_repeats','shared_route125_reuse_pipeline4_compile_repeats','shared_route125_reuse_pipeline4_compilemax_repeats','shared_route125_reuse_pipeline4_compile_long728','shared_route_holdout','shared_route125_reuse_holdout','shared_route0625_reuse_holdout','recall_toy','recall_bead','recall_replication','scene_release','recent_control','recent_hopper_control','long_sum_regression','long_quality_replication'):
