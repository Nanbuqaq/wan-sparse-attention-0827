...
for required in "${plan}" "${expected}" "${checkpoint_states}"; do
  [[ -f "${required}" ]] || { echo "missing full residual control: ${required}" >&2; exit 3; }
done

IFS=',' read -r -a assigned_gpus <<<"${CUDA_VISIBLE_DEVICES}"
lane_count=${#assigned_gpus[@]}
expected_lanes=$(python - "${plan}" <<'PY'
import json, sys
print(json.load(open(sys.argv[1], encoding="utf-8"))["lane_count"])
PY
)
if [[ ${lane_count} -ne ${expected_lanes} ]]; then
  echo "full residual runner received ${lane_count} GPUs, plan requires ${expected_lanes}" >&2
  exit 2
fi
...
PY
  ...
  [[ ${#suites[@]} -gt 0 ]] || { echo "empty full residual lane ${lane}" >&2; return 2; }
  local statuses=()
  local suite_index=0
  set +e
...
python scripts/merge_case_states.py \
  "${state_inputs[@]}" \
  --expected "${expected}" \
  --fill-missing-reason "full residual runner did not emit a terminal state" \
