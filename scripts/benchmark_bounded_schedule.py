...
class Replay:
    def __init__(self, query, exact, archive, groups, capacity, backend="torch_fp32"):
    ...
    def run(self, policy, overlap):
        ...
        with torch.cuda.stream(compute):
            queries = [self.query[:, g*group_q:(g+1)*group_q] for g in range(3)]
            if self.backend == "torch_fp32":
                queries = [q.float() for q in queries]
        ...
        def update(group, key, value):
            ...
            if self.backend == "fa2_bf16":
                part, lse, debug_mask = self.fa2(queries[group].transpose(0, 1).unsqueeze(0),
                    key.transpose(0, 1).unsqueeze(0), value.transpose(0, 1).unsqueeze(0),
                    dropout_p=0., causal=False, return_attn_probs=True)
                # With zero dropout, this pinned API returns LSE but does not
                # materialize the debugging Q-by-K softmax matrix.
                if debug_mask.numel():
                    raise RuntimeError("unexpected dense debugging softmax allocation")
                lse = lse[0]
                next_m = torch.maximum(m, lse)
                alpha, beta = torch.exp(m-next_m), torch.exp(lse-next_m)
                states[group] = [next_m, denominator*alpha + beta,
                    accumulator*alpha[..., None] + part[0].transpose(0, 1).float()*beta[..., None]]
                return
...
def main():
    ...
    parser.add_argument("--partial-backend", choices=("torch_fp32", "fa2_bf16"), default="torch_fp32")
    parser.add_argument("--profile-policy", choices=("query_major", "kv_major", "windowed_union"))
    ...
    for reuse in ("disjoint", "medium", "high"):
        ...
        for capacity in (4, args.pages):
            replay = Replay(query, exact, archive, groups, capacity, args.partial_backend)
            for policy in ("query_major", "kv_major", "windowed_union", "eager_union"):
                ...
                for overlap in (False, True):
                    ...
                    if args.profile_policy == policy and reuse == "high" and capacity == 4 and overlap == args.profile_overlap:
                        torch.cuda.cudart().cudaProfilerStart()
                        with torch.cuda.nvtx.range("dataflow_bounded_" + policy + ("_overlap" if overlap else "_serial")):
                            replay.run(policy, overlap)
                            torch.cuda.synchronize()
                    ...
                    row = {"status": "pass", "reuse": reuse, "capacity": capacity, "policy": policy,
                        "partial_backend": args.partial_backend,
                        ...
                        "GPU_peak_allocated_bytes_including_Q_state_and_reference": torch.cuda.max_memory_allocated(),
    ...
    result = {"status": "pass", "gpu": torch.cuda.get_device_name(), "queries": args.queries, "history_pages": args.pages,
        ...
        "partial_backend": args.partial_backend,
        "FP32_reference_independent_of_partial_backend": True,
        "route_digest_scope": "abstract_group_to_tile_graph; match queries/heads/dim/exact separately",
