...
if [[ ${WAVE2_ORDER:-native_first} == candidate_first ]]; then
  ...
  noise=$(python -c 'import json; print(json.load(open("'$INFER_OUTPUT_DIR'/screen/shared125_transition_anchor/summary.json"))["noise_sha256"])')
  python scripts/run_longlive2_native_reference.py "${common[@]}" --output "$INFER_OUTPUT_DIR/screen/native" --wave2-method w2_native --expected-noise-sha256 "$noise"
else
  python scripts/run_longlive2_native_reference.py "${common[@]}" --output "$INFER_OUTPUT_DIR/screen/native" --wave2-method w2_native
  noise=$(python -c 'import json; print(json.load(open("'$INFER_OUTPUT_DIR'/screen/native/summary.json"))["noise_sha256"])')
  python scripts/run_longlive2_native_reference.py "${common[@]}" --output "$INFER_OUTPUT_DIR/screen/shared125_transition_anchor" \
    --wave2-method w2_steady_sparse --wave2-steady-fraction .125 --wave2-selector shared_sum_block64 \
    --wave2-preparation geometry_cache --wave2-route-refresh first_only --wave2-transition-anchor --expected-noise-sha256 "$noise"
fi
