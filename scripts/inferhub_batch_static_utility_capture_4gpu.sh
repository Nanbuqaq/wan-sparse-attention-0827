...
payload = {
    "artifact_id": "static_utility_capture_4gpu_v1",
    "commit": sys.argv[2],
    "expected_lanes": 4,
    "candidates": 5,
    "marginal_candidates_disabled": "H200 held-out transfer-cost MAPE exceeded 15%",
...
run_lane() {
  ...
  CUDA_VISIBLE_DEVICES=${assigned_gpus[${lane}]} \
    ...
    python scripts/evaluate_capture_routing_candidates.py \
      --capture "${capture}" \
      --mode utility --device cuda --query-chunk-size 128 \
      --group-policy legacy_exact_union \
      --transfer-layout block64 --transfer-mode packed_separate \
      --output "${lane_root}/utility.json" \
      >"${lane_root}/utility.log" 2>&1
}
...
for lane, returncode in enumerate(statuses):
    artifact = root / f"lane{lane}" / "utility.json"
    cases.append(
        {
            "lane": lane,
