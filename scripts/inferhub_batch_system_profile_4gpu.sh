...
if [[ ${#assigned_gpus[@]} -ne 4 ]]; then
  echo "system profile batch requires exactly four GPUs" >&2
  exit 2
fi
...
python scripts/build_system_profile_suite.py \
...
run_lane() {
  ...
  CUDA_VISIBLE_DEVICES=${assigned_gpus[${lane}]} \
    ...
    LONGLIVE_CAPTURE_ROUTE_REUSE=1 \
    LONGLIVE_CAPTURE_ROUTE_LAYERS=0,9,19,29 \
    LONGLIVE_CAPTURE_ROUTE_PASSES=5 \
    python scripts/run_loaded_method_suite.py \
      --suite "${control_root}/system_profile_suite.json" \
...
python scripts/merge_case_states.py \
  "${state_args[@]}" \
  --expected "${control_root}/system_profile_expected.json" \
  --fill-missing-reason "system profile lane emitted no terminal state" \
...
if payload["status"] != "pass":
    raise SystemExit(1)
PY
find "${batch_root}" -type f \( -name '*.json' -o -name '*.pt' -o -name '*.log' \) -print0 \
