"""Frozen real 5B read graphs: fused gather versus repeated grouped packing."""
...
from adapters.longlive_sparse.fused_frame_routes import execute_fused_frame_routes
...
def main():
    ...
    for case in args.cases:
        ...
        for c in capture['calls']:
            ...
            actual=execute_fused_frame_routes(q,k,v,plan)
            ...
            if not torch.equal(ref,actual):raise RuntimeError('same single-FA2 route must be bitwise equal')
            del teacher,actual,ref
            samples={x:[] for x in ('repeated_group_pack','fused_frame_pack')}
            ...
            for repeat in range(args.repeats+2):
                ...
                for method in order:
                    ...
                    out=router.execute(q,k,v,plan,timing=ev) if method=='repeated_group_pack' else execute_fused_frame_routes(q,k,v,plan,timing=ev)
            ...
            row=dict(case=case.name,capture_sha256=sha,frame=c['frame'],layer=c['layer'],
                ...
                packed_KV_bytes=dict(repeated=original,fused=shared),
                ...
                online_backend_pack_includes_all_gathers=True)
            rows.append(row);print(json.dumps(row),flush=True)
    result=dict(status='pass',GPU=torch.cuda.get_device_name(),torch=torch.__version__,rows=rows,
        scope='same real graph GPU replay, not video quality or full delivery speedup; fused gather single-FA2 bitwise full-output equivalence',
