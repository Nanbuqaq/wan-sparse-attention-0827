...
def main():
 ...
 for n in POINTS:
  z=x[:n]; vals=[once(z) for _ in range(3)]; km=float(np.median([v[0] for v in vals])); att=float(np.median([v[1] for v in vals])); rows.append({'frames':n,'global_kmeans_ms':km,'window_attention_ms':att}); print(rows[-1],flush=True)
