...
def main():
 ...
 with torch.inference_mode():
  for st in range(0,len(frames),BATCH):
   batch=list(frames[st:st+BATCH]); inp=processor.video_processor(batch,return_tensors='pt'); px=inp.pixel_values_videos.to(model.device,model.dtype); f=model.get_video_features(px).reshape(len(batch),-1,model.config.vision_config.hidden_size).mean(1); fs.append(f.float().cpu())
