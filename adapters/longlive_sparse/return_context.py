...
POLICIES=('full','pin_first','recent_first','both_first','anchor_transition','current_transition')
...
def permitted_native_frames(policy,owners,physical,roles,phase,*,first_return,returning,global_slots):
    ...
    active=returning and (first_return or policy in ('anchor_transition','current_transition'))
    ...
    for position,(slot,role) in enumerate(zip(physical,roles)):
        ...
        mandatory=role['current'] or (slot<global_slots and policy!='current_transition')
        stale=owner[3]!=phase
        drop=stale and not mandatory and (policy in ('both_first','anchor_transition','current_transition')
