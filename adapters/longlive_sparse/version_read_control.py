...
class VersionReadControl(Wave2TemporalBudget):
    def __init__(self,pipe,method,*,version_read,**kwargs):
        ...
        if version_read not in ('old','new'):raise ValueError('unknown fixed version read')
        self.version_read=version_read;self.version_owners={};self.read_indices={};self.read_index_bytes=0
    ...
    def dispatch(self,layer,original,q,k,v,**kwargs):
        ...
        allowed=permitted_version_positions(owners,physical,self.version_owners,self.version_read);removed=set(range(len(physical)))-set(allowed)
        ...
        row.update(actual_K=k.shape[1]-excluded,logical_pairs=q.shape[1]*q.shape[2]*(k.shape[1]-excluded),
            ...
            clean_commit_scope='full permitted version graph',
            GPU_gather_output_bytes=2*(k.shape[1]-excluded)*k.shape[2]*k.shape[3]*k.element_size() if indices is not None else 0)
        return out

    def audit(self):
        result=super().audit();result.update(version_read=self.version_read,version_read_index_H2D_bytes=self.read_index_bytes,
