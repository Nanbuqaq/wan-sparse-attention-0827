...
class NativeEpisodeMemory:
    ...
    def _install(self):
        ...
        destination_start=0 if self.destination=='global' else before['pinned_start']
        if self.destination=='shot' and (before['pinned_len']!=n or destination_start<n or destination_start+n>before['local_end_index']):
            raise RuntimeError('previous-shot destination must be a distinct existing8-frame pinned region')
        destination_end=destination_start+n
        ...
        try:
            if self.mode.startswith('raw_'):
                for c,(key,value) in zip(primary,self.bank):
                    c['k'][:,destination_start:destination_end].copy_(key);c['v'][:,destination_start:destination_end].copy_(value)
                ...
                with torch.random.fork_rng(devices=[primary[0]['k'].device]):
                    ...
                    for c,scratch in zip(primary,pipe.kv_cache_pos):
                        ...
                        c['k'][:,destination_start:destination_end].copy_(scratch['k'][:,end-n:end]);c['v'][:,destination_start:destination_end].copy_(scratch['v'][:,end-n:end])
