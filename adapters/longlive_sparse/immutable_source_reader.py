...
SOURCE_STAGES=('all','no_clean','no_first','no_last','first_only','second_only','last_only','clean_only','first_last','first_clean','last_clean')
...
def source_stage_allows(policy,step):
    ...
    if policy in ('first_last','first_clean','last_clean'):
        return step in {'first_last':(0,3),'first_clean':(0,4),'last_clean':(3,4)}[policy]
...
class ImmutableSourceReader(Wave2TemporalBudget):
    def __init__(self,pipe,method,*,source_policy,source_replay=False,source_backend='concat',context_policy='full',source_order='append',snapshot_window='latest8',source_archive_enabled=True,source_stage_policy='all',clean_cache_witness=False,**kwargs):
        ...
        if not source_archive_enabled:
            if source_policy!='off' or snapshot_window!='latest8' or source_replay:
                raise ValueError('no-archive is restricted to a fixed off policy without snapshot/replay consumers')
            from .no_source_archive import NoSourceArchive
            self.side_archive=NoSourceArchive()
        self.source_archive_enabled=source_archive_enabled
