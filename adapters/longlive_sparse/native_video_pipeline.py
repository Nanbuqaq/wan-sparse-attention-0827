...
class NativeVideoPipeline:
    def __init__(self,vae,unpatchify,sink,*,source_device,target_device,latent_shape,
        ...
        self.output_pinned_bytes=output_elements*4
        ...
        self.pixel_buffer=torch.empty(output_elements,dtype=torch.float32,pin_memory=True)
    ...
    def submit(self,latent,*,start_latent):
        ...
        if self.serial:
            with self.span('serial_control_wait',start_latent=start_latent):self.worker.wait_completed(self.submissions)
    ...
    def _consume(self,slot,job):
        ...
        with torch.cuda.device(self.target),torch.cuda.stream(self.decode_stream),torch.inference_mode():
            ...
            # The worker owns this slot through decoding/encoding. No producer
            # can overwrite it while either DMA or the CPU witness reads it.
            del owned
            ...
            while True:
                ...
                host=self.pixel_buffer[:raw.numel()].view(raw.shape)
    ...
    def finish(self,*,generation_finished_s):
        self.detach();self.worker.finish();self.closed=True;self.decode_stream.synchronize();self.decoder.finish()
