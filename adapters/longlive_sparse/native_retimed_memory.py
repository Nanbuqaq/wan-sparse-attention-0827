...
class NativeRetimedEpisodeMemory(NativeEpisodeMemory):
    ...
    def _install(self):
        ...
        source_frame=self.source_end-self.frames;virtual_frame=self.target-self.frames
        old_offset=float(self.capture['source_temporal_offset']);new_offset=float(self.pipe._dit_model.rope_temporal_offset)
        delta=virtual_frame+new_offset-source_frame-old_offset
        ...
        plan.update(position_policy='archived_temporal_K_rebound_to_recent_virtual_history',
            actual_source_frames=plan['source_frames'],virtual_source_frames=list(range(virtual_frame,self.target)),
            source_phase=old_offset,current_phase=new_offset,temporal_delta=delta,
