...
**最新精简入口：`docs/ACTIVE_HANDOFF_20260910_RESUMED.md`。** 它覆盖下方所有历史运行状态。
用户已恢复论文研究。双卡509完整门禁与真实CUPTI重叠诊断已完成；距离匹配区域控制已完成，未推广三角色方法。
木箱/信封8条Dense筛选已结束：木箱源通过，信封源不合格；不重跑。
VAE布局四模式门禁已结束，新布局均非无损，未推广。original/recent短回归均已通过。
四条木箱v1均在旧白名单参数检查处失败，尚未生成视频；失败保留，不重跑v1。
已统一校验并加入完整CLI回归，694pass/1skip，v2先一条完整runtime门禁再开剩余三条。
v2首条完整门禁已pass，剩余三条在GPU0/1执行（sessions76894/28254），不可重启。
位置对照的质量还没有结论。已有双设备Perfetto新增官方解析器/SQL重叠核验pass。
以精简入口为准。
